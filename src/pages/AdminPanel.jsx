...
const Card = ({ title, subtitle, action, children }) => (
  <section className="min-w-0 rounded-[24px] border border-slate-200 bg-white p-5 shadow-[0_14px_40px_rgba(15,23,42,0.06)] sm:p-6">
    <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div className="min-w-0">
        <h2 className="text-[23px] font-black tracking-[-0.02em] text-slate-950">{title}</h2>
        {subtitle ? <p className="mt-1 text-[13px] font-medium leading-6 text-slate-500">{subtitle}</p> : null}
      </div>
...
const Stat = ({ label, value, meta, icon: Icon, tone }) => {
  ...
  return (
    <div className="rounded-[22px] border border-slate-200 bg-white p-5 shadow-[0_12px_34px_rgba(15,23,42,0.05)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">{label}</p>
          <p className="mt-4 text-[32px] font-black leading-none text-slate-950">{value}</p>
          {meta ? <p className="mt-3 text-[12px] font-semibold leading-5 text-slate-500">{meta}</p> : null}
        </div>
        <div className={`flex h-11 w-11 items-center justify-center rounded-2xl text-white ${toneMap[tone] || toneMap.slate}`}>
...
const NavItem = ({ active, icon: Icon, label, description, onClick }) => (
  <button
    onClick={onClick}
  ...
  >
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0">
        <p className={`text-[13px] font-black ${active ? 'text-white' : 'text-slate-900'}`}>{label}</p>
        <p className={`mt-1 text-[12px] font-medium leading-5 ${active ? 'text-slate-300' : 'text-slate-500'}`}>{description}</p>
      </div>
    </div>
  </button>
);
...
export default function AdminPanel() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f6f8fb] px-3 py-3 sm:px-4 sm:py-4 lg:px-5 lg:py-5">
      <div className="mx-auto max-w-[1440px] overflow-x-hidden">
        <div className="grid gap-5 xl:grid-cols-[272px_minmax(0,1fr)]">
          <aside className="rounded-[28px] border border-slate-200 bg-white p-4 text-slate-900 shadow-[0_18px_55px_rgba(15,23,42,0.06)] sm:p-5 xl:sticky xl:top-5 xl:max-h-[calc(100vh-40px)] xl:overflow-y-auto">
            <div className="rounded-[22px] border border-slate-200 bg-slate-900 p-4 text-white">
              <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-300">Tripzy Admin</p>
              <h1 className="mt-3 text-[28px] font-black leading-[0.96] tracking-[-0.04em] text-white">Operations Control Room</h1>
              <p className="mt-3 text-[13px] font-medium leading-6 text-slate-300">
                A cleaner workspace for launch operations, AI reliability, exports, and early-access control.
              </p>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
              <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-3 text-[18px] font-black text-slate-950">{admin.username}</p>
                <p className="mt-1 text-[12px] font-medium text-slate-500">Backend-protected admin session</p>
              </div>
              <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">Early access</p>
                <p className="mt-3 text-[24px] font-black text-slate-950">{overview?.earlyUsersClaimed || 0}/{overview?.earlyUserLimit || 500}</p>
                <p className="mt-1 text-[12px] font-medium text-slate-500">{overview?.earlyUsersRemaining || 0} slots remaining</p>
              </div>
            </div>
          </aside>

          <div className="min-w-0">
            <header className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_55px_rgba(15,23,42,0.06)] sm:p-7">
              <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
                <div className="min-w-0">
                  <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-500">Control Center</p>
                  <h2 className="mt-4 text-[32px] font-black leading-[0.95] tracking-[-0.04em] text-slate-950 sm:text-[46px]">
                    {currentPage.label}
                  </h2>
                  <p className="mt-3 max-w-[860px] text-[14px] font-medium leading-7 text-slate-500 sm:text-[15px]">
              ...
              <div className="mt-6 grid gap-3 md:grid-cols-3">
                <div className="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-3.5">
                  ...
                  <p className="mt-2 text-[24px] font-black text-slate-950">{overview?.itinerariesGenerated || 0}</p>
                </div>
                <div className="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-3.5">
                  <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">AI spend today</p>
                  <p className="mt-2 text-[24px] font-black text-slate-950">{formatCurrency(overview?.estimatedAiSpendInrToday || 0)}</p>
                </div>
                <div className="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-3.5">
                  <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">Fallback trips</p>
                  <p className="mt-2 text-[24px] font-black text-slate-950">{overview?.fallbackItineraries || 0}</p>
                </div>
              </div>
            </header>
